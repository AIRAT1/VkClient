...
public abstract class BaseFeedPresenter<V extends BaseFeedView> extends MvpPresenter<V>{
    ...
    public enum ProgressType {
        Refreshing, ListProgress, Pagigng;
    }

    public void showProgres(ProgressType progressType) {
        switch (progressType) {
            case Refreshing:
    ...
    public void loadNext(int offset) {
        loadData(ProgressType.Pagigng, offset, NEXT_PAGE_SIZE);
    }
    ...
    public void onLoadingSuccess(ProgressType progressType, List<BaseViewModel> items) {
        if (progressType == ProgressType.Pagigng) {
